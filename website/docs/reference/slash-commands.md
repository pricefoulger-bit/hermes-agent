...
| `/view [project/topic]` | Ask Hermes for a concise checked-off project view: aim, completed work, current state, next steps, execution mode, blockers, best next move, and recommended push depth. Works in CLI and messaging surfaces by routing a Project View prompt through the normal agent path so it can inspect files, git state, and loop artifacts before answering. |
